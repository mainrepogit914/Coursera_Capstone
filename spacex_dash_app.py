...
# Initialize the options list with the 'All Sites' option
options = [{'label': 'All Sites', 'value': 'ALL'}]

# Loop through the unique launch sites and add each launch site as an option
...
def get_pie_chart(entered_site):
    filtered_df = spacex_df  # Initialize with the entire dataframe
    
    if entered_site == 'ALL':
        # Calculate success rate for each launch site
        #site_success_rates = filtered_df.groupby('Launch Site')['class'].mean()
        site_success_rates = filtered_df[filtered_df['class'] == 1].groupby('Launch Site')['class'].count()

        # Create a pie chart figure
        fig = px.pie(
            values=site_success_rates,
            names=site_success_rates.index,
            title=f'Success Rate for All Launch Sites'
        )
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
    
        # Calculate success and failure counts for the filtered dataframe
        ...
        fig = px.pie(
            ...
            category_orders={'names': ['Failure', 'Success']}
        )
        return fig
